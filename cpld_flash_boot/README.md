...
The FPGA is still configured in Passive Serial mode. During power-up, the CPLD acts as the configuration controller:
...
6. Pass the external Passive Serial programming header through to the FPGA so it can still be programmed manually.
...
pulses, deselects the flash, and switches `DATA0`, `nCONFIG`, `DCLK`, and
open-drain `CONF_DONE` over to the same header pins used by `cpld_passthrough`.
